...
function normalizeThread(row: any): InboxThread {
  return {
    ...
    profile_picture_url: row.profile_picture_url || row.profilePictureUrl || null,
    is_group: row.is_group ?? row.isGroup ?? false,
    group_name: row.group_name || row.groupName || null,

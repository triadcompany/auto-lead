...
export function useNewLeadNotification(userId?: string) {
  ...
  useEffect(() => {
    ...
    return on('lead:created', (lead: any) => {
      if (lead?.seller_id !== userId && lead?.sellerId !== userId) return;
  ...
  }, [userId, audioEnabled, vibrationEnabled, notificationsGranted, on]);
  ...
  return { audioEnabled, vibrationEnabled, notificationsGranted, playNewLeadSound, requestNotificationPermission, updatePreferences };
}

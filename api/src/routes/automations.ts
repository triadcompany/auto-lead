...
export default async function automationsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Querystring: { lead_id?: string; status?: string } }>(
    "/followups",
    async (req) => {
      return prisma.followup.findMany({
        where: {
          ...orgScope(req),
          ...(req.query.lead_id && { leadId: req.query.lead_id }),
  ...
  }>("/followups", async (req, reply) => {
    const followup = await prisma.followup.create({
      data: {
        organizationId: req.auth.orgId,
        leadId: req.body.lead_id,
        messageCustom: req.body.message || null,
        scheduledFor: new Date(req.body.scheduled_at),
        assignedTo: req.body.seller_id || req.auth.userId,
        createdBy: req.auth.userId,
        status: "PENDENTE",
      },
    })
  ...
  fastify.patch<{ Params: { id: string }; Body: { status?: string; message?: string; scheduled_at?: string } }>(
    "/followups/:id",
    async (req, reply) => {
      const updated = await prisma.followup.updateMany({
        where: { id: req.params.id, ...orgScope(req) },
        data: {
          ...(req.body.status && { status: req.body.status as any }),
          ...(req.body.message !== undefined && { messageCustom: req.body.message }),
          ...(req.body.scheduled_at && { scheduledFor: new Date(req.body.scheduled_at) }),
          updatedAt: new Date(),
        },
      })
      if (updated.count === 0) return reply.code(404).send({ error: "Not found" })
